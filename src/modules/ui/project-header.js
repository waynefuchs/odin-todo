...
export default class ProjectHeader {
  static create(project = null) {
    ...
    const projectName = document.createElement(isDefaultProject ? 'h2' : 'input');
    if (isDefaultProject) {
  ...
  static turnOffAllToggles(buttonIgnore = null) {
    ...
    toggleButtons
      ...
      .forEach((button) => {
        ...
        const container = document.querySelector(`#${buttonObject.getAttribute('containerID')}`);
        if (container === null) throw new Error('Unable to query button container');
        container.classList.add('hidden');
        buttonObject.classList.remove('cancel');
        buttonObject.setAttribute('title', buttonObject.getAttribute('reset-title'));
